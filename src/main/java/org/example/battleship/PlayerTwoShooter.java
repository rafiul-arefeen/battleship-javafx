...
public class PlayerTwoShooter {
    ...
    private Boolean[][] player1Board = PlayerOne.getBoolGrid1();
    ...
    void initialize() {
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                shooterGrid2.add(button, col, row);
    ...
    void colourButtonBlue(int row, int col) {
        Button button = (Button) shooterGrid2.getChildren().get((row * 9) + col + 1);
    ...
    public void clickProceed(ActionEvent e1) throws IOException {
        ...
        PlayerOneShooter.canTakeTurnTrue();
