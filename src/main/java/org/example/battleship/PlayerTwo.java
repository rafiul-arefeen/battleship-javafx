...
public class PlayerTwo {
    ...
    void placeSubmarine(int row, int col) {
        ...
        if (checkSourceValidity()) {
            ...
            boolGrid2[sourceRow][sourceCol] = true;
            sCount--;
        }
    }

    boolean checkSourceValidity() {
        return !boolGrid2[sourceRow][sourceCol];
    }
    ...
    void placeAndMarkShip() {
        if (sourceRow == destRow) {
            ...
            for (int i = rowStart; i <= rowEnd; i++) {
                ...
                boolGrid2[i][sourceCol] = true;
            }
        }
    }

    void colourButtonBlue(int row, int col) {
        Button button = (Button) grid2.getChildren().get((row * 9) + col + 1);
    ...
    public void clickProceed(ActionEvent e1) throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("PlayerOneShooter.fxml"));
        PlayerOneShooter.initStartingBoard();
        PlayerTwoShooter.initStartingBoard();
        Stage stage = (Stage) ((Node) e1.getSource()).getScene().getWindow();
        Scene scene = new Scene(fxmlLoader.load());
        stage.setScene(scene);
        stage.setTitle("Battleship: Player One");
        stage.show();
    }
}